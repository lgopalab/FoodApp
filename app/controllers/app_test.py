...
def add_to_cart():
	...
	if item_id in session['menu_item_list']:
		...
		return render_template("user/displaycart.html",
		                       item_quantity_list=zip(session['menu_item_list'], session['quantity_list'], rest_name_list,
		                                              menu_item_name_list, menu_item_cost_list), cost=session["order_cost"])
	else:
...
def complete_registration():
	...
	if user_type == "customer":
		...
		zipcode = 0
		if password == cpassword and len(Customer.query.filter_by(email=email).all()) == 0:
			record = Customer(email, user_name, password, address, zipcode)
			db.session.add(record)
			db.session.commit()
			return render_template("register/register_success.html")
		elif len(Customer.query.filter_by(email=email).all()) > 0:
		...
		if password == cpassword and len(RestaurantWhole.query.filter_by(email=email).all()) == 0:
			...
			db.session.commit()
			return render_template("register/register_success.html")
		elif len(RestaurantWhole.query.filter_by(email=email).all()) > 0:
			error = "The owner already exists in the database"
